            ...
            <div class="border border-gray-200 rounded-lg overflow-hidden">
                <div class="bg-gray-50 px-4 py-3 flex flex-wrap gap-2 justify-between items-center text-sm">
                    ...
                    <div class="flex items-center gap-x-2">
                        ...
                        @if($order->payment)
                            <span class="px-2 py-1 text-xs font-semibold rounded-full {{ $paymentStatusClasses[$order->payment->status] ?? 'bg-gray-100 text-gray-800' }}">
                ...
                <div class="p-4">
                    <div class="flex flex-col md:flex-row gap-4">
                        <img src="{{ optional($order->tour)->thumbnail ?? 'https://placehold.co/150x100' }}" alt="{{ optional($order->tour)->name ?? '' }}" class="w-full md:w-36 h-28 object-cover rounded-md flex-shrink-0">
                        <div class="flex-grow">
                            ...
                            <div class="mt-2 text-sm text-gray-600 space-y-1">
                                ...
                                @if(optional($order->payment)->status === 'SUCCESS' && optional($order->payment)->paid_at)
                                    <p class="text-green-700"><i class="fa-solid fa-clock w-5 text-gray-500"></i>Thanh toán lúc: <strong>{{ optional($order->payment->paid_at)->format('d/m/Y H:i') }}</strong></p>
                                @endif
                            </div>
                        </div>
