...
<div class="bg-white p-6 rounded-lg shadow-sm border border-gray-200">
    ...
    <nav id="profile-sidebar-nav" class="mt-6 space-y-2">
        <a href="{{ route('client.profile') }}" class="profile-nav-link flex items-center gap-x-3 px-4 py-3 rounded-lg text-sm font-semibold transition-colors {{ request()->routeIs('client.profile') ? 'bg-[var(--color-primary-light)] text-[var(--color-primary-dark)]' : 'text-gray-600 hover:bg-gray-100 hover:text-gray-800' }}">
            <i class="fa-solid fa-user-pen w-5 text-center"></i>
            <span>Hồ sơ cá nhân</span>
        </a>
        <a href="{{ route('client.profile.history') }}" class="profile-nav-link flex items-center gap-x-3 px-4 py-3 rounded-lg text-sm font-semibold transition-colors {{ request()->routeIs('client.profile.history') ? 'bg-[var(--color-primary-light)] text-[var(--color-primary-dark)]' : 'text-gray-600 hover:bg-gray-100 hover:text-gray-800' }}">
            <i class="fa-solid fa-receipt w-5 text-center"></i>
            <span>Đơn hàng của tôi</span>
        </a>
        @if($user->account_type === 'LOCAL')
            <a href="{{ route('client.profile.change-password') }}" class="profile-nav-link flex items-center gap-x-3 px-4 py-3 rounded-lg text-sm font-semibold transition-colors {{ request()->routeIs('client.profile.change-password') ? 'bg-[var(--color-primary-light)] text-[var(--color-primary-dark)]' : 'text-gray-600 hover:bg-gray-100 hover:text-gray-800' }}">
